...
void FUNC_MEAS(UART_HandleTypeDef * huart,osMessageQId * Queue_ISENSEHandle_ptr){
	UART_PUTS(huart, "currents and voltage analog values for propulsion module :\r\n");
	char sensor_char[30];
	uint32_t sensor_int, tmp_val2;
	float sensor_f,tmp_val1;
	osEvent sensor_event;
	while(HAL_UART_Receive(huart,RX_Buffer, 1, 1000U)!=HAL_OK){
		sensor_event = osMessageGet(*Queue_ISENSEHandle_ptr,0);
        sensor_int =sensor_event.value.v;
        if(sensor_event.status==osEventMessage){
        	sensor_f = sensor_int*(3.3/4096)/7;
        	sensor_int = sensor_f;
        	tmp_val1 = sensor_f - sensor_int;
        	tmp_val2 = tmp_val1*10000;
        	sprintf(sensor_char, " %d,%d V - ???A - ???W \r", sensor_int,tmp_val2);
        	UART_PUTS(huart,sensor_char);
        }
	}
}
...
void UART_MENU(UART_HandleTypeDef* huart2,int *manual_mode, int *etat_LED,osMessageQId * Queue_PropulsionHandle_ptr,osMessageQId * queue_DirectionHandle_ptr,osMessageQId * queue_ISENSEHandle_ptr){
	...
	/* test if command is steering */
		...
		if (*manual_mode){
			FUNC_MEAS(huart2,queue_ISENSEHandle_ptr);
		}
		else{
			UART_PUTS(huart2, "error: automatic mode enabled, switch to manual mode to use this command\n \r");
		}
